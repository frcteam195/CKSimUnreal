...
const FName ASimPluginPawn::FireRightBinding("FireRight");

ASimPluginPawn::ASimPluginPawn()
{	
...
void ASimPluginPawn::SetupPlayerInputComponent(class UInputComponent* PlayerInputComponent)
{
	...
	if (bCanFire == true)
	{
		...
		if (FireDirection.SizeSquared() > 0.0f)
		{
			...
			if (World != NULL)
			{
				...
				World->SpawnActor<ASimPluginProjectile>(SpawnLocation, FireRotation);
			}

			bCanFire = false;
			World->GetTimerManager().SetTimer(TimerHandle_ShotTimerExpired, this, &ASimPluginPawn::ShotTimerExpired, FireRate);
...
void ASimPluginPawn::ShotTimerExpired()
{
	bCanFire = true;
}
